...
int count_it=1;
int num_training_counts=20000;
float tmp_error=0;
float t_error=0;


#define NUM_NEURON_1 2;
#define NUM_NEURON_2 4;
#define NUM_NEURON_3 4;
#define NUM_NEURON_4 1;
...
int main(void)
{
    ...
    // Setting the amount of layer in the neural network 
    num_layers=4;

    num_neurons = (int*) malloc(num_layers * sizeof(int));
    memset(num_neurons,0,num_layers *sizeof(int));

    // Setting the amount of neurons in each layer
    num_neurons[0]=NUM_NEURON_1;
    num_neurons[1]=NUM_NEURON_2;
    num_neurons[2]=NUM_NEURON_3;
    num_neurons[3]=NUM_NEURON_4;
    ...
    error = (float *) malloc(num_neurons[num_layers-1] * sizeof(float));
    memset(error,0,num_neurons[num_layers-1]*sizeof(float));

    // Get Training Examples
    printf("Enter 4 training examples in total: \n");
    get_inputs();
...
void train_neural_net(void)
{
    ...
    // Open a file to contain the cost
    FILE *cfPtr= fopen( "error.csv", "w+" );

    // Gradient Descent
    for(it=0;it<num_training_counts;it++)
    {
        for(i=0;i<num_training_ex;i++)
        {
            feed_input(i);
...
void compute_error(int i)
{
    ...
    for(j=0;j<num_neurons[num_layers-1];j++)
    {
        tmp_error = desired_outputs[i][j] - lay[num_layers-1].neu[j].actv;
        error[j] = (tmp_error * tmp_error);
        t_error = t_error + error[j];
    }   

    total_error =  t_error/count_it;
    count_it++;
}
